...
var projectDomainPattern = regexp.MustCompile(`^[A-Za-z0-9](?:[A-Za-z0-9.-]{0,252}[A-Za-z0-9])?$`)

func validateProjectDomain(domain string) error {
	domain = strings.TrimSpace(domain)
	if !projectDomainPattern.MatchString(domain) ||
...
func (c *ControllerV1) Create(ctx context.Context, req *v1.CreateReq) (res *v1.CreateRes, err error) {
	res = &v1.CreateRes{}
	if err = validateProjectDomain(req.Domain); err != nil {
		res.SetError(err)
		return res, nil
	}
